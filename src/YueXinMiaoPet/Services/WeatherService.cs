...
namespace YueXinMiaoPet.Services
{
    public class WeatherService
    {
        ...
        public async Task<WeatherInfo> UpdateWeatherAsync(AppConfig config)
        {
            ...
            {
                amapException = ex;
                LogService.Warn("高德地图天气不可用，将切换 Open-Meteo：" + ex.Message);
            }

            try
            {
                WeatherInfo info = await UpdateOpenMeteoAsync(config).ConfigureAwait(false);
                SaveRegionalCache(config, info);
                LogWeatherResult("Open-Meteo 备用", info);
                return info;
            }
            catch (Exception ex)
            {
                LogService.Error("高德地图与 Open-Meteo 均不可用，使用当前行政区缓存。高德错误=" +
                    (amapException == null ? "无" : amapException.Message), ex);
        ...
        private async Task<WeatherInfo> UpdateOpenMeteoAsync(AppConfig config)
        {
            ...
            OpenMeteoResponse response = Deserialize(await _httpClient.GetStringAsync(url).ConfigureAwait(false));
            if (response == null || response.CurrentWeather == null)
            {
                throw new InvalidOperationException("Open-Meteo 返回为空。");
            }
        ...
        private bool HasValidCoordinates(WeatherLocationCacheEntry location)
        {
