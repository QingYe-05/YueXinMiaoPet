...
中国地区天气统一使用高德地图天气实况。程序直接使用省、市、县区选择结果中的行政区代码（adcode）查询高德接口，不再请求其他天气数据源。设置窗口通过“省/直辖市 → 市/州/盟 → 县/区”三级行政区选择天气地区，并按行政区代码隔离天气缓存，避免同名区县或切换城市后错误复用旧天气。天气仍是可选弱干扰模块：
...
设置窗口调试面板会显示实际使用的数据源与数据更新时间。网络失败、断网、Key 不可用或地区信息不可用时不会导致程序崩溃：程序只使用当前行政区对应的高德缓存；没有当前地区缓存时回退到 `unknown`，不会借用其他城市天气。
