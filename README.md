...
仓库同时包含一个独立的 Linux 实验版，位于 `linux/yuexinmiao-pet-linux`，使用 Python 3 + PyQt6 实现，不影响 Windows WPF 主项目和 Windows 安装包。
...
中国地区天气采用“高德地图天气实况优先、Open-Meteo 备用”的双数据源方案。程序直接使用省、市、县区选择结果中的行政区代码（adcode）查询高德实况；高德接口超时、Key 不可用、数据缺失或实况超过 3 小时时，自动解析县区经纬度并查询 Open-Meteo。设置窗口通过“省/直辖市 → 市/州/盟 → 县/区”三级行政区选择天气地区，并按行政区代码缓存定位和天气，避免同名区县或跨城市缓存混用。天气仍是可选弱干扰模块：
...
设置窗口调试面板会显示实际使用的数据源与数据更新时间。网络失败、断网或地区信息不可用时不会导致程序崩溃：高德源失败会自动切换 Open-Meteo；两个数据源均失败时，只使用当前行政区对应的缓存；没有当前地区缓存时回退到 `unknown`，不会借用其他城市天气。

高德 Web 服务 Key 可通过 `AppConfig.AmapWeatherApiKey`、环境变量 `YUEXINMIAO_AMAP_KEY`、安装目录 `PetAssets/amap.key` 或 `%AppData%\YueXinMiaoPet\amap.key` 提供。Key 文件已加入 Git 忽略规则，日志不会记录 Key。
