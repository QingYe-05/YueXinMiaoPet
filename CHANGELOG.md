...
- 修复 Win11 高 DPI 环境下拖动坐标错位、透明区域点击不稳定和重置位置偏向右下的问题。
- 拖动坐标统一转换为 WPF 逻辑单位，并将桌宠限制在鼠标所在显示器工作区内。
- Windows 与 Linux 中国地区天气统一使用高德地图天气实况，不再混用其他天气数据源。
- 直接使用省、市、县区选择结果中的行政区代码查询高德实况；接口异常时仅回退到当前行政区缓存。
- 设置调试信息增加天气数据源和实况更新时间，便于排查天气现象差异。
- 更新跨平台项目结构和文档说明。
...
- 天气服务：高德地图实况、省市选择、经纬度高级选项、断网安全回退。
